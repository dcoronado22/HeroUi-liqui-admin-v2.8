export enum OperacionEstado {
  ...
  function parseEstado(raw: number | string): OperacionEstado | null {
    if (typeof raw === "number") return OperacionEstado[raw] ? raw as OperacionEstado : null;
    const key = Object.keys(OperacionEstado).find(k => k.toLowerCase() === String(raw).toLowerCase());
    return key ? (OperacionEstado as any)[key] as OperacionEstado : null;
  }
  
  export function operacionStateToUI(raw: number | string): { label: string; pct: number; tone: Tone } {
